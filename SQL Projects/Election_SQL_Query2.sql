...
WHERE s.State = 'Uttar Pradesh' AND cr.Constituency_Name = 'FAIZABAD';
...
WHERE cr.Constituency_Name = 'VARANASI'
...
FROM constituencywise_results cr
JOIN partywise_results p ON cr.Party_ID = p.Party_ID JOIN statewise_results sr 
ON cr.Parliament_Constituency = sr.Parliament_Constituency JOIN states s ON sr.State_ID = s.State_ID
